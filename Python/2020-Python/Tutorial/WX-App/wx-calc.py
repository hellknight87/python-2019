...
class Interface(wx.Frame):
    ...
    def on_button_press(self, e):
        ...
        # Handle the even based on the button pressed
        if label == '=': # Calculate the result of the input in the TextCtrl
            # Ignore on empty calculation
            if not calculation:
                return
            ...
            try:
                ...
                return
            except NameError as err: # Catch any manually typed errors (e.g. 2xthree)
                ex.LogError('There was an error. Please check your input and try again.')
                return
